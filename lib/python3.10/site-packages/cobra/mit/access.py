...
class MoDirectory(object):
    ...
    def queryWithCursor(self, queryObject):
        """
        Queries the MIT for a specified object with Cursor. The queryObject provides a
        variety of search options.
        """
        return self._accessImpl.getWithCursor(queryObject)
    ...
    def lookupByClassWithCursor(self, classNames, parentDn=None, **queryParams):
        """
        A short-form managed object (MO) query by class.
        This is to be used to query ephemeral MOs.
        This returns a cursor to access the next set of MOs in
        ...
        if parentDn:
            ...
            dnQuery.queryTarget = 'subtree'
            self.__setQueryParams(dnQuery, queryParams)
            mos, cursor = self.queryWithCursor(dnQuery)
        else:
            classQuery = ClassQuery(classNames)
            self.__setQueryParams(classQuery, queryParams)
            mos, cursor = self.queryWithCursor(classQuery)
        return mos, cursor
