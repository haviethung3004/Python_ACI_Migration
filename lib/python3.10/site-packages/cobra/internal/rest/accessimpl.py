...
import requests
from cobra.internal.codec.jsoncodec import fromJSONStr, parseJSONError, fromJSONStrWithCursor
from cobra.internal.codec.xmlcodec import fromXMLStr, parseXMLError, fromXMLStrWithCursor
...
class RestAccess(object):
    ...
    def getWithCursor(self, request):
        """Return data from the server for the given request on the
        given session with Cursor
        Args:
            request (DnQuery/ClassQuery/TraceQuery/AbstractQuery child): Query
                object
        Return:
        ...
        if rsp.status_code != requests.codes.ok:
            dummyCursor = None
            return self.__parseError(rsp, QueryError, rsp.status_code), dummyCursor
        return self.__parseResponseWithCursor(rsp)
    ...
    def __parseResponseWithCursor(self, rsp):
        if self._session.formatType == AbstractSession.XML_FORMAT:
            return fromXMLStrWithCursor(rsp.text)
        return fromJSONStrWithCursor(rsp.text)
